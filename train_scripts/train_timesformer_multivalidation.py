...
class CFG:
    ...
    comp_folder_name = './'
    comp_dataset_path = f'./'
    exp_name = 'patchsize8_fixdataset_multivalid'
    # ============== model cfg =============
    in_chans = 26 # 
    # ============== training cfg =============
    size = 64
    ...
    lr = 3e-5
    # ============== fold =============
    valid_ids = []  # Replace valid_id = None
    # ============== fixed =============
...
def read_image_mask(fragment_id,start_idx=17,end_idx=43, CFG=CFG):
    ...
    for i in idxs:
        ...
        images.append(image)
    images = np.stack(images, axis=2) if len(images) > 0 else np.zeros((CFG.tile_size, CFG.tile_size, CFG.in_chans))
...
def worker_function(fragment_id, CFG):
    ...
    # Check if the fragment is in validation set
    is_validation = fragment_id in CFG.valid_ids

    for a in y1_list:
        for b in x1_list:
            if not np.any(fragment_mask[a:a + CFG.tile_size, b:b + CFG.tile_size]==0):
                if is_validation or (not np.all(mask[a:a + CFG.tile_size, b:b + CFG.tile_size]<0.05)):
                        ...
                        for xi in range(0,CFG.tile_size,CFG.size):
                            ...
                            if is_validation:
                                if (y1,y2,x1,x2) not in windows_dict:
                                    ...
                                    valid_xyxys.append([x1, y1, x2, y2, fragment_id])  # Add fragment_id to track source
                                    assert image[y1:y2, x1:x2].shape==(CFG.size,CFG.size,CFG.in_chans)
...
class RegressionPLModel(pl.LightningModule):
    def __init__(self, pred_shapes, size=256, with_norm=False):
        super(RegressionPLModel, self).__init__()

        self.save_hyperparameters()
        
        # Dictionary to store predictions for each fragment
        self.mask_preds = {fid: np.zeros(shape) for fid, shape in pred_shapes.items()}
        self.mask_counts = {fid: np.zeros(shape) for fid, shape in pred_shapes.items()}
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for i in range(batch_size):
            x1, y1, x2, y2, fragment_id = xyxys[i]  # Now xyxys is a list of arrays
            pred = F.interpolate(y_preds[i].unsqueeze(0).float(), scale_factor=16, mode='bilinear').squeeze(0).squeeze(0).numpy()
            self.mask_preds[fragment_id][y1:y2, x1:x2] += pred
            self.mask_counts[fragment_id][y1:y2, x1:x2] += np.ones((self.hparams.size, self.hparams.size))

        self.log("val/total_loss", loss1.item(), on_step=True, on_epoch=True, prog_bar=True)
        return {"loss": loss1}
    
    def on_validation_epoch_end(self):
        # Process each fragment's predictions
        for fragment_id, pred in self.mask_preds.items():
            count = self.mask_counts[fragment_id]
            # Avoid division by zero
            mask_pred = np.divide(pred, count, out=np.zeros_like(pred), where=count!=0)
            # Log each fragment's prediction separately
            wandb_logger.log_image(key=f"masks_{fragment_id}", images=[np.clip(mask_pred, 0, 1)], caption=[f"{fragment_id} probs"])
            
        # Reset masks for next validation epoch
        for fragment_id in self.mask_preds:
            self.mask_preds[fragment_id].fill(0)
            self.mask_counts[fragment_id].fill(0)
...
# Add this function to handle the custom batch processing
def custom_collate_fn(batch):
    ...
    for item in batch:
        ...
        masks.append(item[1])
        xyxys.append(item[2])  # This includes fragment_id as string
    
    # Stack images and masks as tensors
    images = torch.stack(images)
    masks = torch.stack(masks)
    
    # Keep xyxys as a list without stacking
    return images, masks, xyxys


# At the end of the file, replace the validation loop with:
torch.set_float32_matmul_precision('medium')

# Define validation fragments
...
# Set all validation fragments at once
CFG.valid_ids = validation_fragments

# Create a dictionary to store validation ground truth shapes
pred_shapes = {}
for fragment_id in validation_fragments:
    try:
        valid_mask_gt = cv2.imread(CFG.comp_dataset_path + f"train_scrolls/{fragment_id}/{fragment_id}_inklabels.png", 0)
        pred_shapes[fragment_id] = valid_mask_gt.shape
    except Exception as e:
        print(f"Error loading validation mask for {fragment_id}: {e}")

# Generate a run name based on all validation fragments
run_slug = f'training_scrolls_valid={"_".join(validation_fragments)}_{CFG.size}x{CFG.size}_submissionlabels'

# Get train and validation datasets using all validation fragments
...
wandb.login(key='06e0055e947922d92266b12aaa9c769b4861d945')
wandb_logger = WandbLogger(project="vesivus", name=run_slug+f'timesformer_patchsize8_fixdataset_multivalid')
model = RegressionPLModel(pred_shapes=pred_shapes, size=CFG.size)
...
trainer = pl.Trainer(
    ...
    callbacks=[
        ModelCheckpoint(
            filename=f'timesformer_wild16_multi_validation_fr'+'{epoch}',
