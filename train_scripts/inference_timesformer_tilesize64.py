...
class RegressionPLModel(pl.LightningModule):
    def __init__(self,pred_shape,size=16,enc='',with_norm=False):
...
def predict_fn(test_loader, model, device, test_xyxys, pred_shape):
    ...
    mask_count_kernel = np.ones((CFG.tile_size, CFG.tile_size))  # Use tile_size instead of size
    kernel = gkern(CFG.tile_size, 1)  # Generate kernel with tile_size
    kernel = kernel / kernel.max()
    model.eval()
    kernel_tensor = torch.tensor(kernel, device=device)  # Move to GPU
    ...
    for step, (images, xys) in tqdm(enumerate(test_loader), total=len(test_loader)):
        ...
        y_preds_resized = F.interpolate(y_preds.float(), scale_factor=16, mode='bilinear')  # (B, 1, 64, 64)
        y_preds_multiplied = y_preds_resized * kernel_tensor  # Now both are 64x64
        y_preds_multiplied_cpu = y_preds_multiplied.squeeze(1).cpu().numpy()

        for i, (x1, y1, x2, y2) in enumerate(xys):
            mask_pred[y1:y2, x1:x2] += y_preds_multiplied_cpu[i]
            mask_count[y1:y2, x1:x2] += mask_count_kernel  # Now mask_count_kernel is 64x64

    mask_pred /= np.clip(mask_count, 1, None)
    return mask_pred

if __name__ == "__main__":
    # Loading the model
    torch.serialization.add_safe_globals([torch.optim.lr_scheduler.CosineAnnealingLR, torch.optim.AdamW, collections.defaultdict, dict])
    model = RegressionPLModel.load_from_checkpoint(args.model_path, strict=False, weights_only=False)
